...
type Defer = { defer: true };
type Historical = { historical: true };
type ConvergeIncoming = { converge: true; incoming: true };
type ConvergeCurrent = { converge: true; current: true };
type State = { state: true };
type InvalidCRDT = { err: string };
...
function HAM(
  ...
  if (incomingState === currentState) {
    let incoming = Lexical(incomingValue) || '';
    let current = Lexical(currentValue) || '';
    if (incoming === current) {
